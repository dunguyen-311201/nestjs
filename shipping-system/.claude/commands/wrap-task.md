...
2. Confirm every business rule / use case this task touched actually has
   a test for both the happy path and the guard-failure (`422` with
   `{ rule: "BR-XX", message }`) per the TDD rule in `docs/lld/00-conventions.md`.
   If coverage is missing, add it — do not mark it done with gaps.
