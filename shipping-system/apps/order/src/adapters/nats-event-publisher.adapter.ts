...
export class NatsEventPublisher implements IEventPublisher {
  ...
  async publish(
    ...
    // NatsRecordSerializer calls .encode() on the headers object, so it
    // must be a real nats-package MsgHdrs (built via headers()), not a
    // plain object - a plain object silently fails at publish time.
