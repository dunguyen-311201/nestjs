...
> [!NOTE]
> **No remaining "no outbox" gap**: every service that publishes NATS events after its own DB write now does so via a Transactional Outbox (Order Creation since the start, Courier as of task 6.1's follow-up, Hub/Sortation built in from day 1 in task 6.2) — see `docs/lld/courier-service.md` v1.4 and `docs/lld/hub-service.md` v1.2. Line-haul and Dispatcher (tasks 6.4/6.5, not yet built) should follow the same pattern rather than reintroducing a synchronous in-request publish.
>
