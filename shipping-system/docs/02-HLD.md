...
> [!WARNING]
> **Accepted MVP risk — no outbox on Hub/Sortation**: Hub/Sortation (task 6.2, not yet built) is planned to publish its NATS events synchronously in the same request as its own DB write, with no outbox. If the NATS publish fails after the DB commit, that scan event is permanently missing from `TRACKING_EVENT`, silently breaking the "100% append-only audit log" target. Accepted for this 16-day slice; production hardening would extend the outbox pattern to this service too. **Courier Service no longer has this gap** — task 6.1's follow-up added a Transactional Outbox (`shipping_courier_db.OUTBOX`, same pattern as Order Creation's) after this exact risk was raised by a supporter reviewer; see `docs/lld/courier-service.md` v1.4.
>
