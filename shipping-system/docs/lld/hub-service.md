...
| :--- | :--- | :--- | :--- |
| v1.3 | 2026-08-06 | Du Nguyen | Added `damaged` flag to `POST /hubs/{id}/receive`, the first and only trigger for `PARCEL.state = Damaged` in this slice — publishes `parcel.damaged` instead of the normal scan event; extended `TRACKING_EVENT.event_type` CHECK constraint with `'DAMAGED'`. Closes progress-report open item #4. |
...
| `damaged` | boolean, nullable | when `true`, hub staff is reporting physical damage found during this scan instead of a normal receive/arrival — see below |

**Response `201`**: `{ status: "recorded" }` — the underlying `parcel.hub_received`/`parcel.arrived_at_hub`/`parcel.misrouted`/`parcel.damaged` publish is async via the Outbox/poller (v1.2), so there is no `event`/`event_id`/`published_at` to return synchronously; which event(s) actually get published is still server-computed from `PARCEL.route_id` vs. the scanning hub's zone (client never sets this directly), except `damaged: true` which always short-circuits straight to `parcel.damaged`. No `tracking_event_id`: this service never writes `TRACKING_EVENT` itself (Tracking does, asynchronously, after consuming the event). **Errors**: `404` hub/parcel not found · `422 BR-08` prepaid parent order not yet `Confirmed` — parcel routed to a holding area, not rejected outright.
...
**Side effect (Damaged)**: when `damaged: true`, this is the *only* trigger for `PARCEL.state = Damaged` in this slice (`docs/reference/progress-report-2026-08-06.md` open item #4, resolved 2026-08-06) — no weight/route/misroute logic runs for this call, and no normal receive/arrival event is published, just `parcel.damaged { parcel_id, hub_id }`. Consumed by Order (`ParcelStateMachine.markDamaged`, valid from any non-terminal state) and Tracking (appends a `DAMAGED` `TRACKING_EVENT` row). `TRACKING_EVENT.event_type` CHECK constraint extended with `'DAMAGED'` (`db/init-db.sql`).
