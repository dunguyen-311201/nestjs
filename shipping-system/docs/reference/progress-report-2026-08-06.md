...
2. **Abandoned prepaid payment** — no auto-cancel mechanism yet for a Stripe checkout session started but never completed. / Chưa có cơ chế tự hủy khi checkout Stripe bị bỏ dở giữa chừng.
3. **UC-15 — passive lost-parcel detection** — by design a parcel is "lost" only when it fails to be scanned at the next hub (no manifest count, per scope cut); an active SLA-sweep job to surface these cases has not been built. / Theo thiết kế, một kiện hàng chỉ được coi là "thất lạc" khi không được quét ở hub kế tiếp (không đếm qua manifest vì đã cắt khỏi scope); job quét SLA chủ động để phát hiện các trường hợp này chưa được xây dựng.
4. **`Damaged` terminal state** — the trigger condition into this state is still undocumented. / Điều kiện kích hoạt trạng thái `Damaged` vẫn chưa được tài liệu hóa rõ ràng.
