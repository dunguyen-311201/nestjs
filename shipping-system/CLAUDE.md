...
git -C "$(git rev-parse --show-toplevel)" rm -rf --quiet --ignore-unmatch shipping-system/docs shipping-system/.claude shipping-system/.gemini
...
- `--ignore-unmatch` is required: if any listed path has nothing to remove (e.g. `.claude` unchanged since the last sync), plain `git rm` exits fatal on that pathspec and removes NOTHING — the merge's re-added `docs/` files then survive to the guard (caught 2026-07-16).
...
- `docs/10-authz-plan.md` — role-based authorization plan (customer/shipper/hub_staff/dispatcher/admin): Clerk publicMetadata roles in the session JWT, gateway ROUTE_ACCESS enforcement, x-user-role propagation, customer ownership via SHIPMENT_ORDER.created_by_user_id; 3-day estimation with explicit cuts
