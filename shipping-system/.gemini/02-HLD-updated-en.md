...
This document describes the high-level system topology, microservice divisions, NATS event patterns, and REST endpoints for the Shipping System, including Stripe payments and Email notifications.
...
Asynchronous communication between services uses the following subjects:

| Subject | Publisher | Consumer | Payload Details |
| :--- | :--- | :--- | :--- |
| `order.created` | `order-service` | `payment-service`, `notification-service` | Emitted when a new order is saved. Triggers Stripe checkout session creation and mail confirmation. |
